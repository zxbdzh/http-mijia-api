...
# 取消登录
curl -X POST http://localhost:8000/api/v1/auth/login/cancel
...
# 获取单个设备信息
curl http://localhost:8000/api/v1/devices/{did}

# 获取设备型号信息（属性列表、动作列表）
curl http://localhost:8000/api/v1/devices/{did}/info

# 获取设备属性
curl http://localhost:8000/api/v1/devices/{did}/properties

# 获取指定属性
curl http://localhost:8000/api/v1/devices/{did}/properties/power

# 设置属性（布尔值示例）
curl -X PUT http://localhost:8000/api/v1/devices/{did}/properties/power \
     -H "Content-Type: application/json" \
     -d 'true'

# 执行设备动作
curl -X POST http://localhost:8000/api/v1/devices/{did}/actions/start
```
...
```bash
# 获取场景列表
curl http://localhost:8000/api/v1/scenes

# 执行场景
curl -X POST http://localhost:8000/api/v1/scenes/{scene_id}
```
